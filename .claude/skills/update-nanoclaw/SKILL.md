...
**Preflight**: checks for clean working tree (`git status --porcelain`). If `upstream` remote is missing, asks you for the URL (defaults to `https://github.com/onecli/nanoclaw.git`) and adds it. Detects the upstream branch name (`main` or `master`).
...
- Ask the user for the upstream repo URL (default: `https://github.com/onecli/nanoclaw.git`).
